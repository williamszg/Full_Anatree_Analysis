#define EnhancedCCCohTResolution_cxx
#include "EnhancedCCCohTResolution.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
TH1D *hTTrue = new TH1D("hTTrue", "True |t| After 2 Tracks", 1000, 0, 1.0);
TH1D *hTConeAngleTrue = new TH1D("hTConeAngleTrue", "True |t| After Cone Angle", 1000, 0, 1.0);
TH1D *hTDoCATrue = new TH1D("hTDoCATrue", "True |t| After DoCA", 1000, 0, 1.0);
TH1D *hTPionCandidateTrue = new TH1D("hTPionCandidateTrue", "True |t| After Pion Candidacy", 1000, 0, 1.0);
TH1D *hTOATrue = new TH1D("hTOATrue", "True |t| After Opening Angle", 1000, 0, 1.0);
TH1D *hTTTrue = new TH1D("hTTTrue", "True |t| After |t|", 1000, 0, 1.0);

TH1D *hTDelta = new TH1D("hTDelta", "Reco |t| Minus True |t| After 2-Tracks", 1000, -1.0, 1.0);
TH1D *hTCADelta = new TH1D("hTCADelta", "Reco |t| Minus True |t| After Cone Angle", 1000, -1.0, 1.0);
TH1D *hTDoCADelta = new TH1D("hTDoCADelta", "Reco |t| Minus True |t| After DoCA", 1000, -1.0, 1.0);
TH1D *hTPCDelta = new TH1D("hTPCDelta", "Reco |t| Minus True |t| After Pion Candidacy", 1000, -1.0, 1.0);
TH1D *hTOADelta = new TH1D("hTOADelta", "Reco |t| Minus True |t| After Opening Angle", 1000, -1.0, 1.0);
TH1D *hTTDelta = new TH1D("hTTDelta", "Reco |t| Minus True |t| After |t|", 1000, -1.0, 1.0);
// ---------------------------------------
...
void EnhancedCCCohTResolution::Loop()
{
   // ===============================================
   ...
   for (Long64_t jentry=0; jentry<numberofentries;jentry++) {
      ...
      double Trk1EnergyTrue = 0;
      double Trk2EnergyTrue = 0;
	 ...
	 if (Distance(reco_nu_vtx_sce_x, reco_nu_vtx_sce_y, reco_nu_vtx_sce_z, trk_sce_start_x_v->at(i), trk_sce_start_y_v->at(i), trk_sce_start_z_v->at(i)) <= NumTrksWithin10Cut) {
	    ...
	    // ---------------------------------------
            ...
            // --- MC True Information ---
            // ---------------------------
            for (int l = 0; l < mc_pdg->size(); l++) {
	       if (mc_pdg->at(l) == 13) { 
                  p3.SetXYZ(mc_px->at(l), mc_py->at(l), mc_pz->at(l));
	          Trk1EnergyTrue = pow(pow(p3.Mag(),2) + pow(0.105,2),0.5);
	          if (p3.Z() < 0) {
		     p3.SetX(-1*mc_px->at(l));
		     p3.SetY(-1*mc_py->at(l));
	             p3.SetZ(-1*mc_pz->at(l));
	          }
               }
	       //if (mc_pdg->at(l) != 14 && mc_pdg->at(l) != 13) { 
	       if (mc_pdg->at(l) == 211) { 
                  p4.SetXYZ(mc_px->at(l), mc_py->at(l), mc_pz->at(l));
	          Trk2EnergyTrue = pow(pow(p4.Mag(),2) + pow(0.13957,2),0.5);
	          if (p4.Z() < 0) {
		     p4.SetX(-1*mc_px->at(l));
		     p4.SetY(-1*mc_py->at(l));
	             p4.SetZ(-1*mc_pz->at(l));
	          }
               }
            }
            // ---------------------------
	 ...
	 if (NumTrksWithin10 == NumTrksWithin10Cut) {
	    ...
	    hVertexActivityFor2TracksNew->Fill(TotalVertexActivityNew); // Filling in the new VA Plot after updating the ana module for the rectangular prism
            ...
            double t_true = pow(Trk1EnergyTrue+Trk2EnergyTrue-p3.Z()-p4.Z(),2) + pow((p3.X()+p4.X()),2) + pow((p3.Y()+p4.Y()),2);
	    hT->Fill(t);
            hTTrue->Fill(t_true);
            hTDelta->Fill(t-t_true);
	    ...
	    if (SavedConeAngle <= ConeAngleCut) {
	       ...
	       hTConeAngle->Fill(t);
	       hTConeAngleTrue->Fill(t_true);
	       hTCADelta->Fill(t-t_true);
	       ...
	       if (DoCA_TrkStarts < DoCACut) {
	          NumEventsWithDoCA++;
		  hTDoCA->Fill(t);
	          hTDoCATrue->Fill(t_true);
	          hTDoCADelta->Fill(t-t_true);
		  ...
		  if (PionCandidateMuonChi2 < PionCandidateMuonCut && PionCandidateProtonChi2 > PionCandidateProtonCut) {
		     ...
		     if (PionCandidateLLRPIDScore > LLRCut) {
		        ...
		        hTPionCandidate->Fill(t);
	                hTPionCandidateTrue->Fill(t_true);
	                hTPCDelta->Fill(t-t_true);
			...
			NumEventsWithOA++;
		        ...
		        hTOA->Fill(t);
	                hTOATrue->Fill(t_true);
	                hTOADelta->Fill(t-t_true);
   ...
   //TFile *TMCInfo = new TFile("MC_Histograms_TResolution.root", "CREATE");
   TFile *TMCInfo = new TFile("CCCoh_Enhanced_Histograms_TResolution.root", "RECREATE");
   ...
   hTTrue->Write();
   hTConeAngleTrue->Write();
   hTDoCATrue->Write();
   hTPionCandidateTrue->Write();
   hTOATrue->Write();
   hTTTrue->Write();
