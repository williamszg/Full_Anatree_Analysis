#define CCCohDaughters_cxx
#include "CCCohDaughters.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
TH1D *hRecoVA0 = new TH1D("hRecoVA0", "The Vertex Activity from Reconstructed Information in Plane 0", 1000, 0, 10000);
TH1D *hRecoVA1 = new TH1D("hRecoVA1", "The Vertex Activity from Reconstructed Information in Plane 1", 1000, 0, 10000);
TH1D *hRecoVA2 = new TH1D("hRecoVA2", "The Vertex Activity from Reconstructed Information in Plane 2", 1000, 0, 10000);
TH1D *hRecoVAAll = new TH1D("hRecoVAAll", "The Vertex Activity from Reconstructed Information from all Planes", 1000, 0, 10000);
...
void CCCohDaughters::Loop()
{
   ...
   //Long64_t Nentries = 20000; // <-- Comment this out to run over entire file!
   ...
   for (Long64_t jentry=0; jentry<Nentries; jentry++) {
	 ...
	 if (Event == event && Run == run && Subrun == subrun && InteractionType == 3 && CCNC == 0 && is_track) {
      ...
      if (mc_vtx_contained == 1) {
	    ...
	    if (distance <= VACut) {
	       VARecoAll = VARecoAll + hit_charge[i];
	       if (hit_plane[i] == 0) VAReco0 = VAReco0 + hit_charge[i];
	       if (hit_plane[i] == 1) VAReco1 = VAReco1 + hit_charge[i];
	       if (hit_plane[i] == 2) VAReco2 = VAReco2 + hit_charge[i];
	    }
         }
      }
