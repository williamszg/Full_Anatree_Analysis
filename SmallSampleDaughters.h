...
// Mon Apr 13 05:14:25 2020 by ROOT version 6.10/02
// from TTree Daughters/Daughters Tree
// found on file: NuCCanalyser_CCCoh_Small_Sample.root
//////////////////////////////////////////////////////////

#ifndef SmallSampleDaughters_h
#define SmallSampleDaughters_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   UInt_t          ntrkspshits;
   UInt_t          ntrkhits;
   Float_t         track_sps_pos[1422][3];   //[ntrkspshits]
   Int_t           track_hit_plane[1422];   //[ntrkhits]
   Float_t         track_hit_charge[1422];   //[ntrkhits]
   ...
   SmallSampleDaughters(TTree *tree=0);
   virtual ~SmallSampleDaughters();
...
#ifdef SmallSampleDaughters_cxx
SmallSampleDaughters::SmallSampleDaughters(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("NuCCanalyser_CCCoh_Small_Sample.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("NuCCanalyser_CCCoh_Small_Sample.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("NuCCanalyser_CCCoh_Small_Sample.root:/NuCCanalyzer");
...
SmallSampleDaughters::~SmallSampleDaughters()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t SmallSampleDaughters::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t SmallSampleDaughters::LoadTree(Long64_t entry)
{
...
void SmallSampleDaughters::Init(TTree *tree)
{
...
Bool_t SmallSampleDaughters::Notify()
{
...
void SmallSampleDaughters::Show(Long64_t entry)
{
...
Int_t SmallSampleDaughters::Cut(Long64_t entry)
{
...
#endif // #ifdef SmallSampleDaughters_cxx
