...
// Wed Apr 22 13:37:41 2020 by ROOT version 6.12/06
// from TTree Daughters/Daughters Tree
// found on file: /pnfs/uboone/persistent/users/zwilliam/cccoh_zgrw_new_sample/cccoh_zgrw_full/NuCCanalyser_CCCoh_BKGD_2s.root
//////////////////////////////////////////////////////////

#ifndef TwoBKGDDaughters_h
#define TwoBKGDDaughters_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   UInt_t          ntrkspshits;
   UInt_t          ntrkhits;
   Float_t         track_sps_pos[4506][3];   //[ntrkspshits]
   Int_t           track_hit_plane[4506];   //[ntrkhits]
   Float_t         track_hit_charge[4506];   //[ntrkhits]
   ...
   TBranch        *b_ntrkspshits;   //!
   TBranch        *b_ntrkhits;   //!
   TBranch        *b_track_sps_pos;   //!
   TBranch        *b_track_hit_plane;   //!
   TBranch        *b_track_hit_charge;   //!
   ...
   TwoBKGDDaughters(TTree *tree=0);
   virtual ~TwoBKGDDaughters();
...
#ifdef TwoBKGDDaughters_cxx
TwoBKGDDaughters::TwoBKGDDaughters(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/pnfs/uboone/persistent/users/zwilliam/cccoh_zgrw_new_sample/cccoh_zgrw_full/NuCCanalyser_CCCoh_BKGD_2s.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/pnfs/uboone/persistent/users/zwilliam/cccoh_zgrw_new_sample/cccoh_zgrw_full/NuCCanalyser_CCCoh_BKGD_2s.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("/pnfs/uboone/persistent/users/zwilliam/cccoh_zgrw_new_sample/cccoh_zgrw_full/NuCCanalyser_CCCoh_BKGD_2s.root:/NuCCanalyzer");
...
TwoBKGDDaughters::~TwoBKGDDaughters()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t TwoBKGDDaughters::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t TwoBKGDDaughters::LoadTree(Long64_t entry)
{
...
void TwoBKGDDaughters::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("ntrkspshits", &ntrkspshits, &b_ntrkspshits);
   fChain->SetBranchAddress("ntrkhits", &ntrkhits, &b_ntrkhits);
   fChain->SetBranchAddress("track_sps_pos", track_sps_pos, &b_track_sps_pos);
   fChain->SetBranchAddress("track_hit_plane", track_hit_plane, &b_track_hit_plane);
   fChain->SetBranchAddress("track_hit_charge", track_hit_charge, &b_track_hit_charge);
...
Bool_t TwoBKGDDaughters::Notify()
{
...
void TwoBKGDDaughters::Show(Long64_t entry)
{
...
Int_t TwoBKGDDaughters::Cut(Long64_t entry)
{
...
#endif // #ifdef TwoBKGDDaughters_cxx
