...
// Thu May 28 13:37:47 2020 by ROOT version 6.10/02
...
#ifndef SignalDaughters_h
#define SignalDaughters_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   UInt_t          ntrkspshits;
   UInt_t          ntrkhits;
   Float_t         track_sps_pos[1558][3];   //[ntrkspshits]
   Int_t           track_hit_plane[1558];   //[ntrkhits]
   Float_t         track_hit_charge[1558];   //[ntrkhits]
   ...
   SignalDaughters(TTree *tree=0);
   virtual ~SignalDaughters();
...
#ifdef SignalDaughters_cxx
SignalDaughters::SignalDaughters(TTree *tree) : fChain(0) 
{
...
SignalDaughters::~SignalDaughters()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t SignalDaughters::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t SignalDaughters::LoadTree(Long64_t entry)
{
...
void SignalDaughters::Init(TTree *tree)
{
...
void SignalDaughters::Show(Long64_t entry)
{
...
Int_t SignalDaughters::Cut(Long64_t entry)
{
...
#endif // #ifdef SignalDaughters_cxx
